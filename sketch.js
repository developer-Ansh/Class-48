...
function preload(){
   ...
   //mario animation img
   marioImg = loadAnimation("images/run-1.png","images/run-2.png","images/run-3.png","images/run-4.png");
...
function spawnBowser()
  {
    if(frameCount % 80 === 0) 
        {
          //to spawn bowser
          var bowser = createSprite(399,330,10,10);
...
function splash() 
{
  ...
  image(green2,width/2,height/2,width,height);
  textFont(marioFont);
  fill(255);
  stroke(0);
  strokeWeight(10);
  ...
  text('Use Arrow keys to move LEFT & RIGHT &Press space to JUMP',width/2,290);
